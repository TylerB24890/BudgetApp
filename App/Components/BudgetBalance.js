      ...
      <View style={styles.screenHeader}>
        <View style={styles.starting}>
					<TouchableOpacity onPress={() => this.props.navigation.navigate('SettingsScreen', { starting: this.state.starting })}>
						<Text style={styles.screenTitle}>Starting Balance: <Text style={[styles.starting, {fontWeight: '500'}]}>{CurrencyFormat(this.state.starting)}</Text></Text>
					</TouchableOpacity>
        </View>
        <View style={styles.available}>
          <Text style={styles.screenTitle}>Available: <Text style={{color: this.state.balanceColor, fontWeight: '500'}}>{CurrencyFormat(this.state.balance)}</Text></Text>
        </View>
