      ...
      <View style={styles.screenHeader}>
        <View style={styles.starting}>
					<TouchableOpacity onPress={() => this.props.navigation.navigate('SettingsScreen', { starting: this.state.starting })}>
						<Text style={styles.screenTitle}>Starting Balance: <Text style={styles.starting}>${this.state.starting.toFixed(2)}</Text></Text>
					</TouchableOpacity>
        </View>
        <View style={styles.available}>
          <Text style={styles.screenTitle}>Available: <Text style={{color: this.state.balanceColor}}>${this.state.balance.toFixed(2)}</Text></Text>
        </View>
